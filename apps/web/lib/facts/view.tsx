...
export type FactRow = {
 ...
 unit:string|null; status:string; programme_id?:string|null; deadline_type?:string|null; immigration_rule_id?:string|null; valid_from:string|null; valid_until:string|null; reviewed_at:string|null;
...
export const factSelect = "id,document_id,programme_id,deadline_type,immigration_rule_id,topic,subject,predicate,value,unit,status,valid_from,valid_until,reviewed_at,evidence(source_url,excerpt,retrieved_at),documents!facts_document_id_fkey(title,sources(name,source_tier))";
...
export function FactCard({fact}:{fact:FactRow}) {
 ...
 return <article className="rounded-2xl bg-surface p-6 shadow-[0_1px_2px_rgb(0_0_0/0.04)] ring-1 ring-hairline">
   ...
   {deadlineLabel(fact.deadline_type)&&<p className="mt-2"><Badge>Loại deadline: {deadlineLabel(fact.deadline_type)}</Badge></p>}
